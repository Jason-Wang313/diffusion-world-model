...
- Learned raw scoring at `N=64`: selected imagined score `1.067`, selected real utility `-0.754`, high-`N` regret `0.896`, imagined-real tail gap `1.460`, deployment gate `block_high_n`.
- Learned denoising ensemble losses decrease over 5 epochs; the primary member decreases from `0.296` to `0.133`.
...
- Learned pilot repair at `N=64`: budget `32` closes `94.1%` of the raw-to-oracle gap on held-out learned diffusion-world-model conditions.
